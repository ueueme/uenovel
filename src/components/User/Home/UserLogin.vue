...
async function onLogin() {
  ...
  if (!loginInfo.username || !loginInfo.password || !loginInfo.code) return Snackbar.info('用户名,密码,验证码不能为空')

  const userCheck = /^[a-zA-Z0-9]{2,16}$/;
  const pwdCheck = /^(?![a-zA-Z]+$)(?!\d+$)(?![^\da-zA-Z\s]+$).{3,20}$/;
  if (!userCheck.test(loginInfo.username)) return Snackbar.warning('用户名长度3到16位不包含中文和特殊字符')
  if (!pwdCheck.test(loginInfo.password)) return Snackbar.warning('密码必须包含(字母和数字),长度6-20位')
