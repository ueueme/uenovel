<template>
  <div class="read-sidebar">
    <div class="read-sidebar-top" :class="{ 'read-sidebar-add': isSidebar }">
      <div class="read-sidebar-item">
        <div class="read-sidebar-left">
          <left theme="outline" size="24" fill="#fff" strokeLinejoin="bevel" strokeLinecap="square"
            @click.stop="$router.back()" />
          <span>{{ chapterTitle }}</span>
        </div>
        <div class="read-sidebar-right">
          <config theme="outline" size="24" fill="#fff" strokeLinejoin="bevel" strokeLinecap="square"
            @click.stop="showPopover = !showPopover" />
          <home theme="outline" size="24" fill="#fff" strokeLinejoin="bevel" strokeLinecap="square"
            @click.stop="onLink" />
          <like theme="outline" size="24" fill="#fff" strokeLinejoin="bevel" strokeLinecap="square"
            @click.stop="addIsCollect" v-if="!isCollect" />
          <like theme="outline" size="24" fill="#ff3992" strokeLinejoin="bevel" strokeLinecap="square"
            @click.stop="addIsCollect" v-else />
        </div>
      </div>
      <div class="read-sidebar-site-box" v-show="showPopover">
        <div class="read-sidebar-site-color">
          <span v-for="color in backgroundColor" :style="{ backgroundColor: color }"
            @click.stop="onChangeColor(color)"></span>
        </div>
    ...
    <div class="read-sidebar-bottom" :class="{ 'read-sidebar-reduce': isSidebar }">
      ...
      <span @click.stop="backNovel">扉页</span>
...
.read-sidebar-bottom {
  ...
  left: 0; // 左侧与页面边缘对齐
  right: 0; // 右侧与页面边缘对齐
  z-index: 999; // 堆叠顺序，确保在其他元素之上
  display: flex; // 使用flex布局
  justify-content: center; // 内容居中对齐
  align-items: center; // 项目垂直居中对齐
  background-color: rgba(0, 0, 0, .8); // 背景颜色，半透明黑色
  transition: .1s; // 过渡效果，持续0.5秒

  /**
   * 定义阅读侧边栏底部的文本样式
   * 该样式类用于设置阅读侧边栏底部文本的颜色、字体大小、对齐方式等
   */
  span {
    color: #fff; // 文本颜色为白色
    flex: 1; // 文本占据剩余空间
    text-align: center; // 文本居中对齐
    font-size: 16px; // 字体大小为18px
    padding: 20px 10px; // 内边距为20px 25px
  }
}
