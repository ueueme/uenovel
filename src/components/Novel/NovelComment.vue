<template>
  <div class="novel-comment">
    <div class="novel-comment-box">
      ...
      <div class="novel-comment-content" style="margin-top:20px;color: #353092;" @click="showPopup(true)">
        <comment theme="outline" size="0.32rem" fill="#000" strokeLinejoin="bevel" strokeLinecap="square" />
        <span>来条善意的评论吧 ~~</span>
      </div>
    </div>
    <div class="novel-comment-list">
      <NovelCommentItem :comment_id="item.id" :user-id="item.user_id" :content="item.comment_content"
        :time="item.createdAt" :praise="item.comment_praise" :reply-count="item.reply_count" :nickname="item.nickname"
        :pic-url="item.picture" v-for="item in commentList" @onShowReply="onShowReply">
      </NovelCommentItem>

      <van-empty description="抢首评，上热搜！" style="text-align: center;" v-if="isNull" />
    </div>
    <!-- 评论输入框 -->
    <var-popup position="bottom" v-model:show="isPopup">
      <div class="novel-comment-from">
        <div class="novel-comment-input">
          <textarea type="text" :placeholder="iptConfig.placeholder" v-model="commentValue" @input="changeValue"
    ...
    <var-popup position="right" v-model:show="isReply">
      <div class="novel-reply-box">
        <div class="novel-reply-title">
          <var-app-bar :title="`共有 ${replyCount} 条评论`" color="#fff" elevation="0" text-color="#000"
            title-position="center" class="user-back">
            <template #left>
              <var-button color="transparent" text-color="#fff" @click="isReply = false" round text>
        ...
        <div class="novel-reply-list" ref="novelReplyList" v-else>
          <NovelCommentItem :user-id="item.user_id" :content="item.comment_content" :time="item.createdAt"
            :praise="item.comment_praise" :is-comment-count="false" :is-comment-reply="false" :nickname="item.nickname"
            :pic-url="item.picture" v-for="item in replyList">
          </NovelCommentItem>
        </div>
